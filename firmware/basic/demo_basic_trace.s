; Demo BASIC - Detailed Trace Version
; Shows INPUT_LEN and X register to debug storage issue
...
@read_line:
    ...
    PHA                     ; Save character
    JSR VEC_CHROUT          ; Echo (may corrupt A)
    PLA                     ; Restore character
    STA INPUT_BUFFER,X      ; Store correct character
    INX
...
@process_command:
    ...
    ; DEBUG: Print X register (length in X before we store it)
    PHA
    TXA                     ; Transfer X to A
    PHA                     ; Save it
    LDX #0
@print_xreg_msg:
    ...
    JMP @print_xreg_msg
@print_xreg_val:
    PLA                     ; Get X value back
    TAX                     ; Restore X immediately
    TXA                     ; Copy to A for printing
    JSR print_hex
    LDA #$0D
    JSR VEC_CHROUT
    LDA #$0A
    ...
    ; DEBUG: Print INPUT_LEN value from memory
    LDX #0
@print_len_msg:
    LDA len_msg,X
    BEQ @print_len_val
...
banner_msg:
    .byte $0D, $0A
    .byte "Demo BASIC v1.3 (Trace)", $0D, $0A
    .byte "Ready", $0D, $0A
    .byte $0D, $0A
    .byte 0

ram_fail_msg:
    .byte "RAM TEST FAILED!", $0D, $0A, 0

xreg_msg:
    .byte "X reg (length): ", 0

len_msg:
    .byte "INPUT_LEN mem: ", 0
